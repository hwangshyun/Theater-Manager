 ...
 */
export async function getMoviesFromKobis(query: string) {
  try {
    ...
    return movies.map(
      (movie: { movieNm: string; movieCd: string; openDt?: string }) => ({
        title: movie.movieNm,
 ...
 */
async function searchMoviesOnTmdb(
  movies: { title: string; movieCd: string }[]
) {
  try {
  ...
  try {
    ...
    return data.posters.map(
      (poster: { file_path: string }) =>
        `${IMAGE_BASE_URL}w500${poster.file_path}`
    );
  } catch (error) {
